ranking_scores <- function(results_source = c("googlesheets", "local"),
  ...
  if(is.na(competition_date)) {
    print("Введите дату соревнований, для которых считаем рейтинг")
    competition_date <- as.character(readline())
  }
  
  if(is.na(competition_coefficient)) {
    print("Введите коэффициент соревнований, для которых считаем рейтинг")
    competition_coefficient <- as.numeric(readline())
  }
  
  print(paste("Считаем рейтинг для даты:", competition_date, "с коэффициентом", competition_coefficient))
